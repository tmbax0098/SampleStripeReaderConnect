...
import { Alert, Button, Heading, Modal, Pressable, Spinner, Text, useToast, VStack } from "native-base";
import {
  Reader,
...
import { PERMISSIONS, PermissionStatus, request, RESULTS } from "react-native-permissions";
...
function DetailOfConnectedReader({ reader, disconnect, update }: DetailOfConnectedReaderProps) {
  ...
  return (
    <VStack bg={"light.50"} p="2" space="1">
      ...
      {reader.availableUpdate && <Button size="lg" onPress={update} colorScheme="success">Update</Button>}

      <Button size="lg" onPress={disconnect}>Disconnect</Button>
    </VStack>
  );
}
...
export default function DiscoverReadersScreen() {
  ...
  const handleConnectBluetoothReader = React.useCallback(async (selectedReader: Reader.Type) => {
  ...
  const configTerminal = async () => {
    ...
    if (Platform.OS === "android") {
      ...
      let result: PermissionStatus = await request(PERMISSIONS.IOS.BLUETOOTH_PERIPHERAL);
  ...
  return (
    <VStack>
      ...
      <Modal isOpen={waitUpdate} size="xs" closeOnOverlayClick={false}>
        <Modal.Content>
          <Modal.Body display="flex" justifyContent="center" alignItems="center">
            ...
            <Text textAlign="center" mt="2" bold>{"progressing %" + updateProgress}</Text>
          </Modal.Body>
        </Modal.Content>
      </Modal>

      <Modal isOpen={waitInitial || waitConnect || waitDisconnect} size="xs" closeOnOverlayClick={false}>
        <Modal.Content>
          <Modal.Body display="flex" justifyContent="center" alignItems="center">
            <Spinner size="lg" mb="4" />
            <Text textAlign="center">
              {waitInitial ? "Wait while initial and discover terminal device ..." : waitConnect ? "Wait connect to reader" : "Wait disconnect"}
            </Text>
            {waitInitial && <Button mt="4" onPress={cancelDiscover}>Finish Search</Button>}
          </Modal.Body>
        </Modal.Content>
      </Modal>
      ...
      {(connectedReader || waitInitial) ? null :
        <Button m="2" size="lg" onPress={configTerminal}>Discover Readers</Button>}

      {connectedReader ?
        <DetailOfConnectedReader reader={connectedReader} disconnect={disconnect} update={installAvailableUpdate} />
        : devices.map((item, index) => (
          <Pressable key={index} onPress={() => handleConnectBluetoothReader(item)} disabled={waitConnect} mx="2"
            mb="2">
            <VStack
              bg="light.100"
              p="2"
              ...
              <Text fontSize="sm">Firmware Version : {item.firmwareVersion}</Text>
              <Text fontSize="sm">Hardware Version : {item.hardwareVersion}</Text>
